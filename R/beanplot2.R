beanplot2 <- function (..., bw = "SJ-dpi", kernel = "gaussian", cut = 3, cutmin = -Inf, 
    cutmax = Inf, grownage = 10, what = c(FALSE, TRUE, TRUE, TRUE), 
    ...
    frame.plot = TRUE, border = NA, innerborder = NA, at = NULL, 
    boxwex = 0.6, ylim = NULL, xlim = NULL, show.names = NA) 
{
    ...
    if (missing(col)) {
        #col <- par("fg")
		color.generator <- function(color.name) brewer.pal(4, color.name)
		col <- lapply(rownames(brewer.pal.info), color.generator)
	}
    if (!is.list(col)) 
        col <- list(col)
    ...
    if (!add) {
        ...
        if (frame.plot) 
            box()
        if (axes)  {# set lwd to par()$lwd to control axis lwd, make axis lwd consistent with par()$lwd
            do.call("axis", c(list(side = 2 - horizontal, lwd=par()$lwd), axespars))
		}
    }
    if (axes) {
        if (is.na(show.names)) 
            show.names <- (n > 1)
        if (show.names) {# set lwd to par()$lwd to control axis lwd, make axis lwd consistent with par()$lwd
            do.call("axis", c(list(1 + horizontal, at = at, labels = names, lwd=par()$lwd), axespars))
		}
    }
